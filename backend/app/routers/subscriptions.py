...
async def renew_subscription(
    ...
    """Renouvelle l'abonnement de l'utilisateur ou de son entreprise"""
    # Récupérer l'abonnement
    subscription = None
    ...
    if current_user.company_id:
        ...
        if current_user.role != "admin":
            raise HTTPException(
                status_code=403,
                detail="Seuls les administrateurs peuvent renouveler l'abonnement"
            )
    ...
    duration = 1 if subscription.type == SubscriptionType.MONTHLY else 12
    renewed_sub = SubscriptionService.renew_subscription(db, subscription.id, duration)
    
    return {"status": "success", "message": "Abonnement renouvelé avec succès"}

@router.post("/license/validate", response_model=LicenseResponseModel)
async def validate_license(
    license_data: LicenseValidateModel,
    db: Session = Depends(get_db)
):
    """Valide une clé de licence"""
    # Valider la licence
    license = SubscriptionService.validate_license(db, license_data.key, license_data.device_id)
    
    if not license:
        raise HTTPException(status_code=400, detail="Clé de licence invalide ou expirée")
    
    return SubscriptionService.license_to_dict(license)